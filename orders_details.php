...
<head>
  ...
  <title>My Canned Drinks Ordering System : Orders Details</title>
  <!-- Bootstrap -->
  ...
  <center>
    ...
    try {
      ...
      $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $stmt = $conn->prepare("SELECT * FROM tbl_orders_a163495, tbl_staffs_a163495,
          tbl_customers_a163495 WHERE
          tbl_orders_a163495.fld_staff_id = tbl_staffs_a163495.fld_staff_id AND
          tbl_orders_a163495.fld_customer_id = tbl_customers_a163495.fld_customer_id AND
          fld_order_id = :oid");
  ...
  <div class="row">
    <div class="col-xs-12 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
      ...
      <div class="form-group">
          ...
          <?php if((isset($_GET['edit'])) && ($editrow['fld_product_id']==$productrow['fld_product_id'])) { ?>
          <option value="<?php echo $productrow['fld_product_name']; ?>" selected><?php echo $productrow['fld_product_name']." ".$productrow['fld_product_name'];?></option>
        <?php } else { ?>
          <option value="<?php echo $productrow['fld_product_id']; ?>"><?php echo $productrow['fld_product_name']." ".$productrow['fld_product_name'];?></option>
        <?php } ?>
      <?php
      } // while
      $conn = null;
  ...
  <div class="row">
    <div class="col-xs-12 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_orders_details_a163495,
            tbl_products_a163495 WHERE
            tbl_orders_details_a163495.fld_product_id = tbl_products_a163495.fld_product_id AND
          fld_order_id = :oid");
          $stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
          $oid = $_GET['oid'];
        $stmt->execute();
